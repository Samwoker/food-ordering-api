use crate::{
    ...
    error::AppError,
    models::user::{
        ForgotPasswordRequest, LoginRequest, RegisterRequest, ResetPasswordRequest, UserResponse,
        UserRole,
    },
    services::auth_service::{
        consume_email_token, consume_reset_token, decode_refresh_token, generate_access_token,
        generate_email_token, generate_refresh_token, generate_reset_token, hash_password,
        revoke_refresh_token, verify_password, AccessClaims,
    },
...
pub async fn register(
    ...
    if let Err(e) =
        email_service::send_verification_email(&user.email, &user.full_name, &email_token, &config)
            .await
    {
        tracing::error!(
            "Failed to send verification email: {}. Verification token: {}",
            e,
            email_token
        );
    }
...
pub async fn login(
    ...
    let access_token =
        generate_access_token(user.id, &user.email, &user.role.to_string(), &config)?;
...
pub fn claims_from_request(req: &HttpRequest) -> Result<AccessClaims, AppError> {
    req.extensions()
        .get::<AccessClaims>()
        .cloned()
