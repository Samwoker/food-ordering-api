...
pub async fn generate_refresh_token(
    ...
    conn.set_ex(refresh_token_key(&jti), user_id.to_string(), ttl_secs).await?;
...
pub async fn revoke_refresh_token(
    ...
    let mut conn = redis.get_async_connection().await?;
    conn.del(refresh_token_key(jti)).await?;
    Ok(())
}
...
pub async fn generate_email_token(
    ...
    let mut conn = redis.get_async_connection().await?;
    conn.set_ex(email_verify_key(&token), user_id.to_string(), EMAIL_TOKEN_TTL).await?;
...
pub async fn generate_reset_token(
    ...
    let mut conn = redis.get_async_connection().await?;
    conn.set_ex(password_reset_key(&token), user_id.to_string(), RESET_TOKEN_TTL).await?;
...
pub async fn consume_reset_token(
    ...
    Uuid::parse_str(&user_id_str).map_err(|_| AppError::Internal)
}